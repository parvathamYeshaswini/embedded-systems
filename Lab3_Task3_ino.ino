...
void loop() {
  ...
  if (stringComplete) {
    ...
    switch (cmd) {
      ...
      case 4:{
        outputString += "";               // everytime we put the original cmd to our outputString, to tell Pi know we get the cmd
  ...
  static unsigned long lastBlink = 0; // remembers time between loop runs
  static bool ledState = false;       // remembers LED on/off

  if (millis() - lastBlink >= 500) {  // half a second passed
    lastBlink = millis();
